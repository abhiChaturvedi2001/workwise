...
export default function Home() {
  ...
  const fetchSeats = async () => {
    try {
      const response = await axios.get(`http://localhost:8060/seats`, {
  ...
  const handleBookSeats = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:8060/seats/reserve`,
  ...
  const handleResetBookings = async () => {
    try {
      const response = await axios.post(`http://localhost:8060/seats/reset`, {
        withCredentials: true,
      });
